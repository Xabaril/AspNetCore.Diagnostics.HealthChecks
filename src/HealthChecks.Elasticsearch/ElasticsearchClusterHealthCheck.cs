...
{
    public class ElasticsearchClusterHealthCheck : IHealthCheck
    {
        private static readonly ConcurrentDictionary<string, ElasticClient> _connections = new();

        private readonly ElasticsearchOptions _options;

        public ElasticsearchClusterHealthCheck(ElasticsearchOptions options)
        {
        ...
        public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                bool isSuccess = healthResponse.ApiCall.HttpStatusCode == 200
                                 && healthResponse.Status is Health.Green or Health.Yellow;
