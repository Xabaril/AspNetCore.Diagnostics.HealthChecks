...
public static class PulsarHealthCheckBuilderExtensions
{
    ...
    {
        builder.Services.AddSingleton(sp => new PulsarHealthCheck(clientFactory(sp), new PulsarHealthCheckOptions
        {
            Topic = topic
        }));
    ...
    {
        builder.Services.AddSingleton(sp => new PulsarHealthCheck(clientFactory(sp), options));
