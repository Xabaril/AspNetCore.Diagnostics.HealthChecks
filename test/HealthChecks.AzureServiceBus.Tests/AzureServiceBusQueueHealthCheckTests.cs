...
public class azureservicebusqueuehealthcheck_should
{
    ...
    [InlineData(true, false)]
    [InlineData(false, false)]
    public async Task can_create_client_with_fully_qualified_endpoint(bool peakMode, bool configureCredential)
    {
        using var tokenSource = new CancellationTokenSource();
    ...
    public async Task reuses_existing_client_when_using_same_fully_qualified_name_with_different_queue(bool peakMode)
    {
        ...
        if (peakMode)
        {
            _clientProvider
                .Received(1)
                .CreateClient(FullyQualifiedName, Arg.Any<DefaultAzureCredential>());
