...
public class cosmosdb_registration_should
{
    ...
    [InlineData("my-cosmosdb-group", null)]
    [InlineData(null, HealthStatus.Degraded)]
    [InlineData("my-azureblob-group", HealthStatus.Degraded)]
    public void add_health_check_with_client_as_parameter(string? registrationName, HealthStatus? failureStatus)
    {
        var client = Substitute.For<CosmosClient>();
        using var serviceProvider = new ServiceCollection()
            .AddHealthChecks()
            .AddCosmosDb(client, name: registrationName, failureStatus: failureStatus)
            .Services
            .BuildServiceProvider();
    ...
    public void add_health_check_with_client_as_parameter_and_single_parameter_delegate(string? databaseId, string? registrationName, HealthStatus? failureStatus, params string[] containerIds)
    {
        var client = Substitute.For<CosmosClient>();
        using var serviceProvider = new ServiceCollection()
            .AddHealthChecks()
    ...
    [InlineData(null, null, null, "first-collection", "second_collections")]
    [InlineData("cosmosdb", "my-azureblob-group", HealthStatus.Degraded, "first-collection", "second_collections")]
    public void add_health_check_with_client_as_parameter_and_double_parameter_delegate(string? databaseId, string? registrationName, HealthStatus? failureStatus, params string[] containerIds)
    {
        var client = Substitute.For<CosmosClient>();
        using var serviceProvider = new ServiceCollection()
            .AddHealthChecks()
